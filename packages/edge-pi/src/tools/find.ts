...
export function createFindTool(options: ToolOptions) {
	...
	return tool({
		...
		execute: async ({ pattern, path: searchDir, limit }, { abortSignal }) => {
			...
			const cmd = `find ${JSON.stringify(searchPath)} -type f | sed 's#^${searchPath}/##' | grep -E ${JSON.stringify(
				pattern.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*").replace(/\./g, "\\."),
			)} | head -n ${effectiveLimit}`;
			const result = await runtime.exec(cmd, { cwd, abortSignal });
			if (result.exitCode !== 0 && !result.output.trim()) return "No files found matching pattern";
